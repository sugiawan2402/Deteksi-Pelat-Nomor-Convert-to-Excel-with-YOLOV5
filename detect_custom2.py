    ...
    for path, img, im0, vid_cap in dataset:
        ...
        for i, det in enumerate(pred):  # detections per image
            if webcam:  # batch_size >= 1
                p, s, im0, frame = path[i], '%g: ' % i, im0[i].copy(), dataset.count
            else:
                p, s, im0, frame = path, '', im0.copy(), getattr(dataset, 'frame', 0)
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    crop1 = im0[int(xyxy[1]):int(xyxy[3]), int(xyxy[0]):int(xyxy[2])]
                    cv2.imshow("plat", crop1)
                    
                    name = pytesseract.image_to_string(crop1, config='-l eng --oem 1 --psm 7 -c tessedit_char_whitelist=ABCDEFGHIJKLMNOPQRSTUVWXYZ1234567890')
                    # # name = name.strip('\n\x0c')
                    if len(name) > 5:
                        print('pytesserct = ', name)
                        markAttendance(name)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--source', type=str, default="vid11.mp4", help='source')  # file/folder, 0 for webcam
    # parser.add_argument('--source', type=str, default="vid.xAX", help='source')  # file/folder, 0 for webcam
